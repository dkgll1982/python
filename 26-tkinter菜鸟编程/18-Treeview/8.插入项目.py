...
def insertItem():
    ...
    city = cityCmb.get()                              #获得cityCmb的输入
    if(len(state.strip())==0 or len(city.strip())==0):#如果数据输入不完全不往下执行
        return 
    id = (id1 if city=='英国' else (
        id2 if city=='德国' else (
        id3 if city=='意大利' else (
        id4 if city=='中国' else (
        id5 if city=='法国' else (
        id6 if city=='美国' else id7 
    ))))))                                            #三目表达式嵌套设置对应哪个组的id
    try:                                              #暂时未找到好的方法判断ID是否存在，这里通过异常处理不存在进行ID的创建
        txt = tree.item(id,'text')
    except:
        id = tree.insert('',index=END,text=city)      #如果ID不存在则进行创建
    tree.insert(id,END,text=state,values=(city),tags='evenColor')      #插入
    stateEntry.delete(0,END)                          #删除stateEntry,因为是单行文本，所以第一个参数就是表示从第几个字符开始，第二个字符表示第几个字符结束
    var.set('')                                       #删除cityCmb
...
stateLab = Label(root,text="品牌:")                  #建立品牌标签
stateLab.grid(row=0,column=0,padx=5,pady=3,sticky=W)
stateEntry = Entry()
stateEntry.grid(row=0,column=1,sticky=W+E,padx=5,pady=3)
cityLab = Label(root,text='国家:')                   #建立国家标签 
cityLab.grid(row=0,column=2,sticky=E)

#state='readonly'控制只能下来选择，不能手动输入
cityCmb = Combobox(root,textvariable=var, state='readonly',value=("英国","德国",'意大利','中国','法国','美国','日本'),)
cityCmb.grid(row=0,column=3,sticky=W+E,padx=5,pady=3)
...
tree = Treeview(root,column=('cities','populations'),selectmode=EXTENDED)
...
id4 = tree.insert('',index=END,text='中国')
id5 = tree.insert('',index=END,text='法国')
id6 = tree.insert('',index=END,text='美国')
id7 = tree.insert('',index=END,text='日本') 
...
tree.grid(row=1,column=0,columnspan=5,padx=10,pady=5,sticky=W+E+N+S)

rmbtn = Button(root,text='删除项目',command=removeItem)
rmbtn.grid(row=2,column=2,padx=10,pady=5,sticky=W)
