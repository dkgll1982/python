...
#建立family optionMenu
var1 = StringVar()
#注意元组的第一个元素表示是默认选中项，后边的则是所有列表项
family = ("隶书","宋体","幼圆","隶书","楷体","魏碑","微软雅黑") 
op1 = OptionMenu(frm,var1,*family,command=familychange) 
op1.pack(side=LEFT)

#建立weight optionMenu
var2 = StringVar()
weight = ("bold","bold","normal")
op2 = OptionMenu(frm,var2,*weight,command=weightchange) 
op2.pack(side=LEFT)

#建立size combobox
var3 = IntVar()
comb = Combobox(frm,textvariable=var3)
sizerang = [x for x in range(8,30)]
comb['value']=sizerang
comb.current(10) 
#设置下拉选中事件
comb.bind("<<ComboboxSelected>>",sizechange)  
comb.pack(side=LEFT)
...
init_font = Font(family=family[0],weight=weight[0],size=comb.get())
...
text1.tag_config('a',background = 'blue',foreground = 'black')
text1.tag_config('b',background = 'orange',foreground = 'purple')
text1.tag_config('c',background = 'brown',foreground = 'pink')

#pack布局：注意先后顺序，此组件铺满除BOTTOM(xscroll)，RIGHT(yscroll)的剩余空间
text1.pack(fill=BOTH,expand=True)           #当窗口放大或缩小时，仍然随整个窗口铺满
#在结尾处添加文本内容
text1.insert(END,txt0,'init')
text1.insert(END,txt1,"a")
text1.insert(END,'\r\n')
text1.insert(END,txt2,"b")
text1.insert(END,'\r\n')
text1.insert(END,txt3,"c")

text1.focus_set()       #设置什么控件获取到焦点，可以看到一个'|'标记在一闪一闪，一般我们用tab键能够切换焦点

text1.tag_add('tag1','5.3','6.6','10.6','10.6+5c')
