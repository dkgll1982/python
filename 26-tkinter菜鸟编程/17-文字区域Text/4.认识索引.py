...
def getselect(): 
    try:
        text1.mark_set('market', '11.9')
        text1.insert('market','插入文字','tag_1')
        selText=text1.get(SEL_FIRST,SEL_LAST)
        print('选取文字：',selText)
        print('索引位置文本',text1.get("market"))
        print('all marks',text1.mark_names())
        print('market:%r,insert:%r,end:%r,cuerrent:%r,selectionStart:%r,selectionEnd:%r'%( 
            text1.index("market"), 
            text1.index(INSERT), 
            text1.index(END), 
            text1.index(CURRENT), 
            text1.index(SEL_FIRST),
            text1.index(SEL_LAST)))
        ...
        #text1.focus_force()
        #改变鼠标光标位置
        text1.mark_set('insert', '9.4') #标记mark(这个名字随意起)在l之前
