...
# ScrolledText组件：以下说明来源于官方文档
# 该ScrolledText模块提供了一个相同名称的类，它实现了一个基本文本小部件，
# 该小部件具有一个垂直滚动条，用于执行“正确的事情”。
# 使用ScrolledText该类比直接设置文本小部件和滚动条要容易得多。
# 构造函数与Tkinter.Text该类的相同。
# 注意：也就是说没有X轴滚动条，仍然需要另外绑定X滚动条！！！
text = ScrolledText(root,undo=True,wrap='none')
