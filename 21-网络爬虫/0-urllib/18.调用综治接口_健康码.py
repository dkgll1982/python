...
class JKMSpider():
    ...
    def get_data(self): 
        ...
        SELECT replace(replace(trim(replace(replace(replace(replace(sfzh,chr(10)),CHR(32)),chr(13),chr(9)),' ')),'	'),'	') sfzh,mzt,mffd,SORT_TYPE FROM (  
            select DISTINCT sfzh, '绿码' mzt,'{}' mffd,1 SORT_TYPE from T WHERE sfzh NOT IN
            (
                select A from excel_table WHERE TYPE='jkm' AND d='绿码' AND c=to_char(sysdate,'YYYY-MM-DD')
            )  
            UNION ALL
            select DISTINCT sfzh,'红码' mzt,'{}' mffd,2 from T WHERE sfzh IN
            (
                select A from excel_table WHERE TYPE='jkm' AND d='绿码' AND c=to_char(sysdate,'YYYY-MM-DD') AND TO_SINGLE_BYTE(JSON_VALUE(b,'$.msg'))='查询失败' 
            )
            AND sfzh NOT IN
            (
            ...
            UNION ALL
            select DISTINCT sfzh,'黄码' mzt,'{}' mffd,3 from T WHERE sfzh IN
            (
                select A from excel_table WHERE TYPE='jkm' AND d='红码' AND c=to_char(sysdate,'YYYY-MM-DD') AND TO_SINGLE_BYTE(JSON_VALUE(b,'$.msg'))='查询失败' 
            ) 
                AND sfzh NOT IN
            (
                select A from excel_table WHERE TYPE='jkm' AND d='黄码' AND c=to_char(sysdate,'YYYY-MM-DD') 
            )  
        ) ORDER BY SORT_TYPE 
    ) WHERE LENGTH(SFZH)=18 AND ROWNUM<""".format(self.key_column,self.datatype,self.file_md5,self.key_column,self.city,self.city,self.city) + str(self.pagecount) 
        sql2 = "" 
        ...
        for row in rows:  
            ...
            sql2 = "INSERT INTO excel_table(TYPE,A,B,C,D) VALUES('jkm','%s','%s','%s','%s')"%(row[0],jsonstr,datetime.datetime.now().strftime('%Y-%m-%d'),row[1])
            cursor.execute(sql2)
    ...
    def imp_excel(self):
        ...
        from excel_table ta 
        join excel_table tb on ta.{}=tb.a 
            and tb.TYPE='jkm' 
            and tb.c=to_char(sysdate,'YYYY-MM-DD') 
            and ta.TYPE='{}' 
            and ta.z='{}' 
...
if __name__ == '__main__': 
    ...
    if len(args) > 1: 
        ...
        if os.path.isfile(file_name): 
            ...
            dcol = jkm.savedb()  
            pagesize = math.ceil(jkm.totalcount/jkm.pagecount)*3 
