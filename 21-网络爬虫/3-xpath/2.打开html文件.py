...
html = etree.parse(r'backup/1.html',etree.HTMLParser())
...
result = html.xpath('//a[@id="a2"]/../@class')  #xpath返回的是一个列表
print(result)
for item in result:
    print(item)
    
print('-'*40)  
result = html.xpath('//img[starts-with(@src,"https")]') 
for item in result:
    print('=.',item)

print('-'*40) 
#遇到属性值有多个的情况我们需要使用contains()函数了，contains()匹配一个属性值中包含的字符串 。包含的字符串，而不是某个值。
result = html.xpath('//img[contains(@src,"20")]') 
for item in result:
    print(item)
