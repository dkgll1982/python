...
class DouTuLaSpider():
    ...
    def parse_content(self, response):
        ...
        for a in a_list:
            ...
            pic_list = a.xpath('.//div[@class="random_article"]//img/@data-backup')
            if title:
                if not os.path.exists(path + title[0]):
                    os.mkdir(path + title[0]) 
                for index, pic in enumerate(pic_list):
                    response = self.send_request(pic)  # 发送图片请求
                    name = str(index + 1) + "_" + pic[-20:]  # 图片名字
                    self.write_content(response, name, path + title[0])
