...
class bdapi:  
    ...
    #百度地图API根据经纬度获取地址
    def get_url(self,query,location):
        
        # 以get请求为例http://api.map.baidu.com/geocoder/v2/?address=百度大厦&output=json&ak=你的ak
        # 地理编码接口
        queryStr = '/place/v2/search?query=%s&output=json&location=%s&ak=%s&radius=2000&output=json'%(query,location,self.ak)
...
if __name__ == '__main__':
    ...
    g = bdapi(ak,sk);
    url = g.get_url('银行',"39.915,116.404")
    print(url)
    addr = g.get_addr(url) 
    print("地址：%s"%(addr))
