...
#坐标计算网格服务
geoserver = 'https://xixian-jczl-konggang.spacecig.com/CIGService/rest/services/0/intersectFeaturesByXY';
...
def get_zb(index,biao):
    ...
    conn = cx_Oracle.connect('cigproxy','cigproxy','136.2.34.65:15231/xe')
    cursor = conn.cursor() 
    ...
    #查询数据的sql
    sql1 =  ("select id,'X='||replace(lat_lng,',','&Y=') ZB from zz_event where g_id=1125899906842624 and trunc(sysdate)<> trunc(update_date) and rownum<="+str(rowcount))      
    sql2 = ""
    ...
    for row in rows:  
        try:  
            ...
            print(dict)
            if dict.get("gridId"): 
                sql2 = "update "+ biao+" set g_id=%d,update_date=to_date('%s','YYYY-MM-DD HH24:MI:SS') where id='%s'"%(int(dict.get("gridId")),update_date,row[0]) 
            else:       
