...
class Matedata_odps:
    ...
    def get_matedata(self, tables): 
        ...
        for row in tables:
            ...
            try:
                ...
                pass
            par = None if l == [] else l[-1] 
            
            ddl_sql = self.get_oracle_ddl(table_name[0:10],table_comment,schema)
            data.append((table_name,table_comment,par,ddl_sql))
            print('查询:{}表({})结构[{}]成功！'.format(table_comment, table_name, par)) 
        return data
    ...
    def write_excel(self,DATA):    
        ...
        sheet.write('A1', '表名', bold)
        sheet.write('B1', '表注释', bold)
        sheet.write('C1', '分区名称', bold)
        sheet.write('D1', 'Oracle表名', bold)
        sheet.write('E1', 'Oracle建表语句', bold) 
           
        rowindex = 0   
        for item in data:
            table_name,table_comment,par,ddl_sql = item
            rowindex += 1
            ...
            #     sheet.write(rowindex, 2, par) 
            #     sheet.write(rowindex, 3, table_name[0:30]) 
            #     sheet.write(rowindex, 4, col[1][0]) 

            book.close()

    # 获取所有存储的odps的表名，并将样例数据写入excel
    def start(self):
        sql = 'SELECT "推送后的表名" table_name FROM test."BASE_表结构0228" WHERE ROWNUM<3'
