...
def mydownloadfile(sourcefile,dir,photoname):
    try:
        if not os.path.exists(dir):
            print("dir:%s not exit!" % (dir)) 
            os.makedirs(dir) 
        #下载图片，并保存到文件夹中
        urllib.request.urlretrieve(sourcefile,filename=dir+'\\'+photoname) 
    except IOError as e:
        print("IOError")
...
def get_data(data_from, type):
    ...
    sql = '''SELECT DATA_FROM,type,FILENAME,'C:\\湖州重点\\'||DATA_FROM||'重点人口_'||type DIR_NAME,NAME||'_'||CARD_NUM||'.'||REPLACE(PNG,'.*.') PHOTO_NAME FROM (
            SELECT PNG,DECODE(TB.DATA_FROM,'cx','长兴','aj','安吉','nth','南太湖','wx','吴兴','nx','南浔','dq','德清') DATA_FROM,
                TB.FOCUS_CONTROL,TB.CARD_NUM,TB.NAME,replace(replace(TA.FILENAME,'new\\',''),
                'C:\\Data\\证照库\\CardImg\\','http://10.21.198.201:8080/image/CardImg/')
                FILENAME,
                DECODE(TC.TYPE,'201','刑满释放','202','社区矫正','203','精神病','204','吸毒','205','艾滋病','206','信访','
            207','重点青少年','208','邪教','209','传销','210','其他','211','危险品从业人员') TYPE
    ...
    for row in rows:
        mydownloadfile(row[2],row[3],row[4])
