...
town_list = ['南浔区','开发区','双林镇','和孚镇','菱湖镇','练市镇']
...
def get_person():
    ...
    for town in town_list:
        sql =  (""" SELECT 身份证号码,姓名,性别,出生日期,户籍地址,现住地址,联系方式,NVL(乡镇,'南浔区') 乡镇,NVL(村社区,'南浔区') 村社区,NVL(四级网格,'南浔区') 四级网格,NVL(网格全称,'南浔区') 网格全称,nvl(网格层级,1) 网格层级,人口类型,重点类型 FROM (
                        SELECT CARD_NUM 身份证号码,NAME 姓名,DECODE(GENDER,'1','男','2','女') 性别,TO_CHAR(BIRTH_DATE,'YYYY-MM-DD') 出生日期,
                            R_ADDR 现住地址,D_ADDR 户籍地址,PHONE 联系方式,DECODE(PERSON_TYPE,'1','户籍人口','2','流动人口') 人口类型, replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(    
                                replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(FOCUS_CONTROL,'301','残疾人'),
                                '302',	'特困儿童'),
                                '303',	'留守人员'),
                                '304',	'老年人'),
                                '305',	'临时救助'),
                        ...
                        WHERE DEL_FLAG=0 AND G_ID is not null and FOCUS_CONTROL is not null
                    ) where NVL(乡镇,'南浔区')='{}'
