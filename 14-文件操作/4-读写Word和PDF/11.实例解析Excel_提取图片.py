...
'''
#参考链接:https://www.cnblogs.com/leanfish/p/9374454.html
...
# 读取解压后的文件夹，打印图片路径
def read_img(zipfile_path,excel_file_path):
    if not isfile_exist(zipfile_path):
        return False
    ...
    for file in file_list:
        ...
        print(filepath)
    #将ZIP文件重新改为excel文件
    os.rename(zipfile_path, excel_file_path)  
...
def compenent(excel_file_path):
    ...
    if zip_file_path != '':
        if unzip_file(zip_file_path):
            read_img(zip_file_path,excel_file_path)
